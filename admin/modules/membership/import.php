...
if (isset($_POST['doImport'])) {
    ...
    if (!$_FILES['importFile']['name']) {
        ...
        while (!feof($file)) {
            ...
            if ($row_count < $record_offset) {
                ...
                if ($field) {
                    ...
                    $first_name = '\''.$field[2].'\'';
                    $last_name = '\''.$field[3].'\'';
                    $gender = ( ! empty($field[4])) ? $field[4] : 0; // patched by Indra Sutriadi
                    $member_type_id = utility::getID($dbs, 'mst_member_type', 'member_type_id', 'member_type_name', $field[5], $mtype_id_cache);
                    $member_email = $field[6]?'\''.$field[6].'\'':'NULL';
                    $member_address = $field[7]?'\''.$field[7].'\'':'NULL';
                    $postal_code = $field[8]?'\''.$field[8].'\'':'NULL';
                    $inst_name = $field[9]?'\''.$field[9].'\'':'NULL';
                    $is_new = $field[10]?$field[10]:'0';
                    $member_image = $field[11]?'\''.$field[11].'\'':'NULL';
                    $pin = $field[12]?'\''.$field[12].'\'':'NULL';
                    $member_phone = $field[13]?'\''.$field[13].'\'':'NULL';
                    $member_fax = $field[14]?'\''.$field[14].'\'':'NULL';
                    $member_since_date = '\''.$field[15].'\'';
                    $register_date = '\''.$field[16].'\'';
                    $expire_date = '\''.$field[17].'\'';
                    $birth_date = $field[18]?'\''.$field[18].'\'':'NULL';
                    $member_notes = preg_replace('@\\\s*'.$field_enc.'$@i', '', $field[19]);
        ...
        // close file handle
        fclose($file);
        utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'membership', 'Importing '.$inserted_row.' members data from file : '.$_FILES['importFile']['name'], 'Import', 'Add');
