...
if ($unauthorized) {
    ...
    $msg .= 'top.location.href = \''.SWB.'index.php?p=admin\';'."\n";
    $msg .= '</script>'."\n";
...
if ($timeout && !isset($_SESSION['remember_me'])) {
    ...
    $msg .= __('Your Login session has timed out.').' <a target="_top" href="'.SWB.'index.php?p=admin" style="text-decoration: underline; color: #000;">'.__('Click here to Login again').'</a>';
    $msg .= '</div>'."\n";
    // unset cookie admin flag
    #setcookie('admin_logged_in', true, time()-86400, SWB);
    #setcookie('admin_logged_in', true, time()-86400, SWB, "", FALSE, TRUE);
