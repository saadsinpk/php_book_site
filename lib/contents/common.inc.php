...
$colltype_q = $dbs->query('SELECT coll_type_name FROM mst_coll_type ORDER BY coll_type_id LIMIT 50');
...
/* GMD List */
ob_start();
echo '<option value="0">'.__('All Formats').'</option>';
$gmd_q = $dbs->query('SELECT gmd_name FROM mst_gmd ORDER BY gmd_id LIMIT 50');
