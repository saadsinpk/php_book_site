...
trait SearchFilter
{
    ...
    {
        $query = DB::getInstance()->query("SELECT mst_gmd.gmd_id AS value, mst_gmd.gmd_name AS label, COUNT(biblio.gmd_id) AS count FROM mst_gmd LEFT JOIN biblio ON biblio.gmd_id = mst_gmd.gmd_id GROUP BY mst_gmd.gmd_id");
        return $query->fetchAll(\PDO::FETCH_ASSOC);
    }
    ...
    public function buildFilter($opac, $filters): string
    {
        ...
        foreach ($this->reOrder($filters) as $index => $filter) {
            ...
            $str .= <<<HTML
                <div class="d-flex justify-content-between align-items-center cursor-pointer" data-bs-toggle="collapse" data-bs-target="#collapse-{$index}">
                    <strong class="text-base">{$filter['header']}</strong>
                    <i class="dropdown-toggle"></i>
                </div>
                <div class="collapse show text-base" id="collapse-{$index}"><div class="mt-2">
HTML;
...
HTML;
                    ...
                    if (count($filter['items']) > 3) {
                        # close collapse items wrapper
                        $str .= '</div>';
                        $str .= '<a class="d-block mt-2" data-bs-toggle="collapse" href="#seeMore-' . $index . '">' . __('See More') . '</a>';
                    }
                    break;
            }
