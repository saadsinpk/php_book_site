        ...
        foreach ($this->record_detail['attachments'] as $attachment_d) {
          ...
          if (!is_null($attachment_d['access_limit'])) {
              ...
              if (!utility::isMemberLogin()) {
                $_output .= '<li class="attachment-locked"><a class="font-italic" href="index.php?p=member&destination=' . (\SLiMS\Url::getSlimsFullUri('#attachment')->encode()) . '">'.__('Please login to see this attachment').'</a></li>';
                continue;
              // member type access check 
                ...
                $_output .= '<li class="attachment-locked cursor-pointer">'. __('You have no authorization to download this file.') . '</li>';
                continue;
              }
          }

          if ($attachment_d['mime_type'] == 'application/pdf') {
            $_output .= '<li class="attachment-pdf" itemscope itemtype="http://schema.org/MediaObject"><a itemprop="name" property="name" '.(utility::isMobileBrowser() ? 'target="_blank"' : 'class="openPopUp"').' title="'.$attachment_d['file_title'].'" href="./index.php?p=fstream&fid='.$attachment_d['file_id'].'&bid='.$attachment_d['biblio_id'].'" width="780" height="520">'.$attachment_d['file_title'].'</a>';
            ...
            $_output .= '</li>';
              ...
              switch ($attachment_d['placement']) {
                  case 'embed':
                      ...
                      break;
                  case 'popup':
                      $_output .= '<li class="attachment-audio-video" itemprop="video" property="video" itemscope itemtype="http://schema.org/VideoObject">'
            ...
            $_output .= '<li class="attachment-audio-audio" itemprop="audio" property="audio" itemscope itemtype="http://schema.org/AudioObject">'
              .'<a itemprop="name" property="name" class="openPopUp" title="'.$attachment_d['file_title'].'" href="./index.php?p=multimediastream&fid='.$attachment_d['file_id'].'&bid='.$attachment_d['biblio_id'].'" width="640" height="480">'.$attachment_d['file_title'].'</a>';
            $_output .= '<div class="attachment-desc" itemprop="description" property="description">'.$attachment_d['file_desc'].'</div>';
            if (trim($attachment_d['file_url']) != '') { $_output .= '<div><a href="'.trim($attachment_d['file_url']).'" itemprop="url" property="url" title="Other Resource Link" target="_blank">Other Resource Link</a></div>'; }
            $_output .= '</li>';
              ...
              switch ($attachment_d['placement']) {
                  case 'embed':
                      ...
                      break;
                  case 'popup':
                      $_output .= '<li class="attachment-url-list" itemscope itemtype="http://schema.org/MediaObject"><a itemprop="name" property="name"  href="'.trim($attachment_d['file_url']).'" title="Click to open link" class="openPopUp" width="560" height="315">'.$attachment_d['file_title'].'</a><div class="attachment-desc">'.$attachment_d['file_desc'].'</div></li>';
                      break;
                  default:
                      $_output .= '<li class="attachment-url-list" itemscope itemtype="http://schema.org/MediaObject"><a itemprop="name" property="name"  href="'.trim($attachment_d['file_url']).'" title="Click to open link" target="_blank">'.$attachment_d['file_title'].'</a><div class="attachment-desc">'.$attachment_d['file_desc'].'</div></li>';
              }
            ...
            $_output .= '<li class="attachment-image" itemprop="image" itemscope itemtype="http://schema.org/ImageObject"><a itemprop="name" property="name" class="openPopUp" title="'.$attachment_d['file_title'].'" href="index.php?p=fstream&fid='.$attachment_d['file_id'].'&bid='.$attachment_d['biblio_id'].'" width="'.$imgwidth.'" height="'.$imgheight.'">'.$attachment_d['file_title'].'</a>';
            ...
            $_output .= '<li class="attachment-image" itemscope itemtype="http://schema.org/MediaObject"><a itemprop="name" property="name" title="Click To View File" href="index.php?p=fstream&fid='.$attachment_d['file_id'].'&bid='.$attachment_d['biblio_id'].'" target="_blank">'.$attachment_d['file_title'].'</a>';
            if (trim($attachment_d['file_url']) != '') { $_output .= ' [<a href="'.trim($attachment_d['file_url']).'" itemprop="url" property="url" title="Other Resource related to this file" target="_blank" style="font-size: 90%;">Other Resource Link</a>]'; }
            $_output .= '<div class="attachment-desc" itemprop="description" property="description">'.$attachment_d['file_desc'].'</div></li>';
          }
        }
...
HTML;
    ...
    public function getItemCopy() {
      ...
      $_output = '<div class="flex flex-col"><div class="w-100 flex flex-row w-full">
                <div class="col-3 flex flex-row border border-gray-300 p-2" style="background:#12659d;color: white; font-weight:600">
                      <span class="mx-2">Location</span>
                </div>
                <div class="col-3 border border-gray-300 p-2" style="background:#12659d;color: white; font-weight:600">
                   <span class="mx-2">Call Number</span> 
                </div>
				<div class="col-3 border border-gray-300 p-2" style="background:#12659d;color: white; font-weight:600">
                   <span class="mx-2">Barcode</span> 
                </div>
                <div class="col-3 border border-gray-300 p-2" style="background:#12659d;color: white; font-weight:600">
      ...
      foreach ($copies as $copy_d) {
        ...
        if ($loan_stat_q->num_rows > 0) {
            ...
            list($avail_class, $avail_status) = ['item-onloan', __('Due ').date($sysconf['date_format'], strtotime($loan_stat_d[0])).''];
            ...
            list($avail_class, $avail_status) = ['item-available', __('On Shelf').(trim($copy_d['item_status_name']??'')?' - '.$copy_d['item_status_name']:'')];
        }

        extract($copy_d);
        $location_name = empty($location_name) ? __('No current location') : $location_name;
        ...
        $_output .= <<<HTML
            <div class="w-100 flex flex-row w-full" style="background:white">
                <div class="col-3 flex flex-row border border-gray-300 p-2">
                    <span class="block mx-2">{$location_name}</span>
                </div>
                <div class="col-3 border border-gray-300 p-2">
                   <span class="block mx-2">{$call_number}</span>
                </div>                <div class="col-3 border border-gray-300 p-2">
                   <span class="block mx-2">{$item_code}</span>
                </div>
                <div class="col-3 border border-gray-300 p-2">
                  <span class="block mx-2">{$avail_status}</span>
                </div>
            </div>			
        HTML;
    ...
    protected function htmlOutput()
    {
        ...
        foreach ($this->record_detail['authors'] as $data) {
          $authors .= '<a href="?author='.urlencode('"'.$data['author_name'].'"').'&search=Search" title="'.__('Click to view others documents with this author').'">'.$data['author_name']."</a><br />";
    ...
    public function MODSoutput()
    {
        ...
        */
        ...
        $_copy_q = $this->db->query(sprintf('SELECT i.item_code, i.call_number, stat.item_status_name, ct.coll_type_name, loc.location_name, stat.rules, i.site FROM item AS i '
    ...
    public function DublinCoreOutput()
    {
		...
		$_copy_q = $this->db->query('SELECT i.item_code, i.call_number, stat.item_status_name, ct.coll_type_name, loc.location_name, stat.rules, i.site FROM item AS i '
            .'LEFT JOIN mst_item_status AS stat ON i.item_status_id=stat.item_status_id '
			.'LEFT JOIN mst_coll_type AS ct ON i.coll_type_id=ct.coll_type_id '
            .'LEFT JOIN mst_location AS loc ON i.location_id=loc.location_id '
            .'WHERE i.biblio_id=%d', $this->detail_id);
